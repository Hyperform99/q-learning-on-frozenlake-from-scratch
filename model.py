...
def init_q_table(num_states, num_actions):
    ...
    qAgentTable = np.zeros((num_states, num_actions), dtype=np.float64)
    return qAgentTable
...
def greedy_action(q_table, state):
    ...
    return list(q_table[state]).index(max(q_table[state]))
...
def td_target(reward, gamma, q_table, next_state, done):
    ...
    return(reward + gamma * (max_q_value(q_table, next_state) if not done else 0))
...
def run_training_episode(env, q_table, epsilon, alpha, gamma, rng, max_steps=200):
    ...
    for i in range(max_steps):
        next_state, rewardState, done = interaction_step(env, q_table, next_state, epsilon, alpha, gamma, rng)
        reward += rewardState
    return(reward)
